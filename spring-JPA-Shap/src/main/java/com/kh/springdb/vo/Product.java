 ...
 *
	...
	Lombok
		...
		@RequireArgsConstructor : 이 어노테이션이 적용된 클래스는 필수로 초기화 해야하는 final필드나 @NotNull로 표시된 필드를 사용하는 생성자를 자동으로 생성해줌
								예제 :
...
JPA 어노테이션
	@Prepersist : 엔티티가 데이터베이스에 저장되기 전에 호출되는 메서드를 지정
			...
			private Date createDate; // now라는 메서드를 활용해서 현재 시간 기입
			@PrePersists
			private void perWork(){
			//저장 전 수행할 작업을 작성
			//생성일자 설정, 특정 필드 초기화

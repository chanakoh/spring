...
import com.kh.ThymeSpring.service.UserService;
...
public class UserController {
	...
	public String updateUserForm(@PathVariable int id, Model model) {
		//사용자 정보를 가져와서 모델에 추가해줘야 하기 때문에 사용자 정보를 가지고오는 모델추가코드를 작성
...
사용법 : @PathVariable int id
	{id}
@ModelAttribute("값:)
	Thyemleaf 뷰에서 설정한 값의 이름을 사용해서 모델 속성에 엑서스 할 수 있음
	엑서스(access): 컴퓨터 데이터 또는 리소스를 어떤 방식으로든 사용할 수 있도록 권한을 주거나 권한이 담겨진 것을 의미
	
@ModelAttribute("user") : user라는 이름으로 Model에 User객체를 추가한 것
...
@RequestMapping : 사용자의 http요청을 특정 메서드와 할 수 있도록 매핑하는(감싸주는) 역할을 함
